...
<form method="post" class="woocommerce-ResetPassword lost_reset_password">
	...
	<h1 class="form-heading" style="font-size: 18px"><?php echo apply_filters( 'woocommerce_reset_password_message', esc_html__( 'Enter a new password below.', 'woocommerce' ) ); ?></h1><?php // @codingStandardsIgnoreLine ?>
	...
	<p class="woocommerce-form-row form-row">
		...
		<button style="background-color: #00FFFF;color: #000;border: none;padding: 12px 48px;border-radius: 35px;font-size: 16px;cursor: pointer;" type="submit" class="woocommerce-Button button<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>" value="<?php esc_attr_e( 'Save', 'woocommerce' ); ?>"><?php esc_html_e( 'Save', 'woocommerce' ); ?></button>
	</p>
